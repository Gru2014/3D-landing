...
const Card: React.FC<CurrentFeaturesProps> = ({ card, isActive }) => {
    return (
        <div className={`rounded-[20px] p-[3px] border-gradient cursor-pointer ${isActive ? 'max-w-[220px] h-[280px] -mt-6' : 'max-w-[170px] h-[230px]'} relative`}>
            <div className={`rounded-[20px] flex flex-col items-center hover:bg-opacity-25  py-5 ${isActive ? 'max-w-[214px] h-[274px] bg-opacity-25' : 'bg-black max-w-[164px] h-[224px]'}`}>
                ...
                <div className={`absolute bottom-2 w-[150px] border-gradient p-0.5 rounded-full  ${isActive?'my-8':'my-1'}`}>
