...
const TokenomicsCard: React.FC<TokenomicsCardPropsType> = ({ card, index, hoverIndexScale, hoverIndexColor }) => {
  return (
    <div className='max-[769px]:w-screen max-[769px]:flex max-[769px]:justify-center'>
      <div className={`transition-all max-[769px]:w-5/6 cursor-pointer ${hoverIndexScale === index ? 'w-[600px] h-[70px] ' : hoverIndexScale === index + 1 || hoverIndexScale === index - 1 ? 'w-[550px] h-[60px]' : 'w-[500px] h-[50px]'}  rounded-[20px] border-gradient-right p-[3px]`}>
